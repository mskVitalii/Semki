...
func IdentityHandler(c *gin.Context) interface{} {
	...
	if !ok {
		err := errors.New("couldn't retrieve userClaims[_id] after authorization")
	...
	// role
	organizationRoleStr, ok := claims["userRole"].(string)
	if !ok {
		lib.ResponseInternalServerError(c,
			errors.New("couldn't retrieve userClaims[OrganizationRole] after authorization"),
			"Invalid claims format")
		return err
	}
	organizationRole := model.OrganizationRole(organizationRoleStr)
	if organizationRole == "" {
		lib.ResponseInternalServerError(c,
			errors.New("couldn't retrieve userClaims[OrganizationRole] after authorization by "+organizationRoleStr),
			"Invalid claims format")
	}
	// organization
	organizationIdStr, ok := claims["OrganizationId"].(string)
	if !ok {
		lib.ResponseInternalServerError(c,
			errors.New("couldn't retrieve userClaims[OrganizationId] after authorization"),
			"Invalid claims format")
		return err
	}
